...
class Stores extends Admin_Controller 
{
    ...
    */
	...
	{
		if(!isset($this->user_permission['viewStore'])) {
	...
	*/
	public function fetchStoresData()
	{
		...
		foreach ($data as $key => $value) {
			...
			if(isset($this->user_permission['updateStore'])) {
				$buttons = '<button type="button" class="btn btn-default" onclick="editFunc('.$value['id'].')" data-toggle="modal" data-target="#editModal"><i class="fa fa-pencil"></i></button>';
			}

			if(isset($this->user_permission['deleteStore'])) {
    ...
    */
	...
	{
		if(!isset($this->user_permission['createStore'])) {
			redirect('dashboard', 'refresh');
		}
