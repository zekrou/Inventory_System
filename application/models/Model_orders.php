...
class Model_orders extends CI_Model
{
	...
	public function create($user_id = null)
	{
		...
		// 🔴 CORRECTION: Récupère les bonnes valeurs avec underscore
	...
	public function update($id, $user_id = null)
	{
		if ($id) {
			...
			$gross_amount = $this->input->post('gross_amount_value');
			$net_amount = $this->input->post('net_amount_value');
			$paid_amount = $this->input->post('paid_amount') ? $this->input->post('paid_amount') : $old_order['paid_amount']; // ✅ Garde l'ancien montant
			$discount = $this->input->post('discount') ? $this->input->post('discount') : 0;
			$due_amount = $net_amount - $paid_amount;
			...
			$customer_id = $this->input->post('customer_id'); // ✅ AVEC underscore
			...
			// ✅ GESTION DU PRICE TYPE OVERRIDE
			$price_type_override = $this->input->post('customer_type_override'); // ✅ AVEC underscore
			$override_reason = $this->input->post('override_reason');

			// Si pas de raison fournie mais override différent du type client
			if (!$override_reason && $price_type_override && $price_type_override != $customer_type) {
				$type_labels = array(
				...
				$override_label = isset($type_labels[$price_type_override]) ? $type_labels[$price_type_override] : $price_type_override;
				$customer_label = isset($type_labels[$customer_type]) ? $type_labels[$customer_type] : $customer_type;
				$override_reason = 'Price override: ' . $override_label . ' (Customer default: ' . $customer_label . ')';
			}
			...
			$data = array(
				'customer_id' => $customer_id,
				'customer_type' => $customer_type,
				'price_type_override' => $price_type_override ? $price_type_override : $customer_type,
				'override_reason' => $override_reason,
				'customer_name' => $customer_name,
				...
				'discount' => $discount,
				'paid_status' => $paid_status,
				'payment_method' => $this->input->post('payment_method') ? $this->input->post('payment_method') : $old_order['payment_method'],
				'payment_notes' => $this->input->post('payment_notes') ? $this->input->post('payment_notes') : $old_order['payment_notes'],
				'user_id' => $user_id
			);
			...
			for ($x = 0; $x < $count_product; $x++) {
				...
				// ✅ AVEC rate_value et amount_value
				$items = array(
					'order_id' => $id,
					'product_id' => $product_id,
					'qty' => $qty_ordered,
			...
			// ✅ ENREGISTRER NOUVEAU PAIEMENT SI MONTANT AUGMENTÉ
			if ($paid_amount > $old_paid_amount) {
				$new_payment = $paid_amount - $old_paid_amount;
				$payment_method = $this->input->post('payment_method') ? $this->input->post('payment_method') : $old_order['payment_method'];
				$payment_notes = $this->input->post('payment_notes') ? $this->input->post('payment_notes') : $old_order['payment_notes'];
				$this->recordPaymentInstallment($id, $new_payment, $payment_method, $payment_notes, $due_amount, $user_id);
			}
